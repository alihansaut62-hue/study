...
function getApiBase() {
  // If frontend is opened from file:// or another host, use local Flask server.
...
async function parseErrorResponse(res) {
  ...
  if (contentType.includes('application/json')) {
    ...
    return data?.error || 'Ошибка сканирования';
  }
  const text = await res.text();
  return text?.trim() || 'Ошибка сканирования';
}
...
function mkHumanList(items) {
  if (!Array.isArray(items) || items.length === 0) {
    return '<li>Рекомендации пока отсутствуют.</li>';
  }
  return items.map(item => `<li>${esc(item)}</li>`).join('');
}

function activateHumanTab(tabName) {
  document.querySelectorAll('.human-tab').forEach(btn => {
    btn.classList.toggle('active', btn.dataset.humanTab === tabName);
  });
  $('human-owner').classList.toggle('active', tabName === 'owner');
  $('human-visitor').classList.toggle('active', tabName === 'visitor');
}

function initHumanTabs() {
  const tabs = document.querySelectorAll('.human-tab');
  tabs.forEach((btn) => {
    btn.onclick = () => activateHumanTab(btn.dataset.humanTab);
  });
}

function renderHumanAdvice(data) {
  const human = data.human_advice || {};
  const risk = human.risk_level || data.risk_level || 'НЕИЗВЕСТНО';

  $('humanOwnerNote').textContent = `Уровень риска для владельца: ${risk}. Ниже простой план действий.`;
  $('humanOwnerList').innerHTML = mkHumanList(human.owner || []);

  $('humanVisitorNote').textContent = human.visitor_verdict || 'Проверяйте HTTPS и не вводите личные данные на сомнительных страницах.';
  $('humanVisitorList').innerHTML = mkHumanList(human.visitor || []);

  activateHumanTab('owner');
}
...
async function startScan() {
  ...
  try {
    ...
    const scanEndpoint = `${getApiBase()}/scan`;
    ...
    if (!res.ok) {
      showErr(await parseErrorResponse(res));
      resetBtn(); return;
    }
    ...
    if (e.name === 'AbortError') {
      showErr('Сканирование заняло слишком много времени. Попробуйте снова.');
    } else {
      showErr('Ошибка соединения — убедитесь, что Python сервер запущен на 127.0.0.1:5000');
    }
    resetBtn();
  }
}
...
function renderResults(data) {
  ...
  /* Защита backend */
  const gb = $('grid-b'); gb.innerHTML = '';
  (data.backend_protection || []).forEach(t =>
    gb.innerHTML += mkCard(t.name, t.status, t.risk, t.exploit, t.fix, false, null));
  $('cnt-b').textContent = (data.backend_protection || []).length + ' рекомендаций';

  /* Для владельца и посетителя */
  renderHumanAdvice(data);
